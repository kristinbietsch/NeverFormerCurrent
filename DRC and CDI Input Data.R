...
keep_surveys <- c("CDIR61FL.DTA", "CIIR62FL.DTA", "CMIR71FL.DTA")
#$ "CDIR61FL.DTA", "CIIR62FL.DTA", "HTIR61FL.DTA", have no calendar and thus different coding for v302a

survey_keep <- surveys_mini %>% filter(IRfile %in% keep_surveys) %>% rename(SurveyName=IRfile) %>% select(SurveyName) 



surveys_mini <- surveys_mini    %>% filter(IRfile %in% keep_surveys) %>% rename(SurveyName=IRfile) %>% select(Country, StartYear, SurveyName)
...
for (row in 1:nrow(surveys_mini)) {
  ...
  women_clean <- women %>% filter(v013>=1 & v013 <=7) %>%  mutate(Status=case_when(v313==1 | v313==2 | v313==3 ~ "Current",
...
write.csv(use_status_clean , "use_status_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(use_age_status_clean, "use_age_status_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(use_mar_status_clean , "use_mar_status_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(never_group_clean , "never_group_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(never_intention_clean , "never_intention_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(never_has_intention_clean , "never_has_intention_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(former_group_clean , "former_group_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(former_intention_clean , "former_intention_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(former_has_intention_clean, "former_has_intention_clean DRC CDI CAM 061923.csv", row.names=F, na="")
#write.csv(former_why_discontinue_clean , "former_why_discontinue_clean DRC CDI 061623.csv", row.names=F, na="")
#write.csv(former_time_use_group_clean , "former_time_use_group_clean DRC CDI 061623.csv", row.names=F, na="")
write.csv(mii_clean , "mii_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(current_group_clean, "current_group_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(current_methodmix_clean, "current_methodmix_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(current_source_clean , "current_source_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(current_timesince_clean , "current_timesince_clean DRC CDI CAM 061923.csv", row.names=F, na="")
write.csv(empowerment_clean , "empowerment_clean DRC CDI CAM 061923.csv", row.names=F, na="")
